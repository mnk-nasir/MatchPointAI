...
### 📊 Startup Evaluation Flow

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': { 'primaryColor': '#0d9488', 'actorBkg': '#0f172a', 'actorBorder': '#14b8a6', 'actorTextColor': '#ccfbf1', 'signalColor': '#5eead4', 'signalTextColor': '#f0fdfa', 'labelBoxBkgColor': '#0f172a', 'labelBoxBorderColor': '#334155', 'labelTextColor': '#94a3b8', 'loopTextColor': '#5eead4', 'noteBkgColor': '#0f172a', 'noteTextColor': '#99f6e4', 'noteBorderColor': '#14b8a6', 'activationBkgColor': '#1e293b', 'activationBorderColor': '#475569', 'sequenceNumberColor': '#f59e0b' }}}%%
sequenceDiagram
    autonumber
    participant F as 🚀 Founder
    participant FE as ⚛️ Frontend
    participant BE as 🎯 evaluation_views
    participant SC as ⚙️ scoring_engine
    participant DB as 🗄️ SQLite

    F->>FE: Complete multi-step evaluation form
    FE->>BE: POST /evaluations/submit/
    BE->>BE: Validate via serializer
    BE->>SC: Run multi-factor scoring model
    Note over SC: Market · Team · Traction · Financials
    SC-->>BE: Score breakdown + rating
    BE->>DB: Persist evaluation record
    BE-->>FE: 201 Created + scores
    FE-->>F: Interactive report with insights
```

### 🔐 Authentication Flow

```mermaid
%%{init: {'theme': 'dark', 'themeVariables': { 'primaryColor': '#f59e0b', 'actorBkg': '#0f172a', 'actorBorder': '#f59e0b', 'actorTextColor': '#fef3c7', 'signalColor': '#fcd34d', 'signalTextColor': '#f0fdfa', 'labelBoxBkgColor': '#0f172a', 'labelBoxBorderColor': '#334155', 'labelTextColor': '#94a3b8', 'noteBkgColor': '#1c1917', 'noteTextColor': '#fde68a', 'noteBorderColor': '#f59e0b', 'activationBkgColor': '#1e293b', 'activationBorderColor': '#475569', 'sequenceNumberColor': '#6366f1' }}}%%
sequenceDiagram
    autonumber
    participant U as 👤 User
    participant FE as ⚛️ Frontend
    participant BE as 🎯 auth_views
    participant JWT as 🔑 SimpleJWT
    participant DB as 🗄️ SQLite

    U->>FE: Register or Login
    FE->>BE: POST /auth/register/ or /auth/login/
    BE->>DB: Validate credentials / Create user
    BE->>JWT: Issue token pair
    JWT-->>BE: access + refresh tokens
    BE-->>FE: 200 OK + tokens
    FE->>FE: Store tokens in memory
    Note over FE: Auto-attach Bearer header
    FE->>BE: GET /auth/me/
